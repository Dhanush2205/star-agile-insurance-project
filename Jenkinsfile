    ...
    stages {
        stage('SCM Checkout') {
            steps {
                ...
                git 'https://github.com/Dhanush2205/star-agile-insurance-project.git'
       ...
       stage("Docker build"){
            steps {
				sh 'docker version'
				sh "docker build -t dhanushbnaidu/insurance-eta-app:${BUILD_NUMBER} ."
				sh 'docker image list'
				sh "docker tag dhanushbnaidu/insurance-eta-app:${BUILD_NUMBER} dhanushbnaidu/insurance-eta-app:latest"
            }
        }
		...
		stage('Push2DockerHub') {
			...
			steps {
				sh "docker push dhanushbnaidu/insurance-eta-app:latest"
			}
		}
